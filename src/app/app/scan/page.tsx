...
export default async function ScanPage({ searchParams }: ScanPageProps) {
  ...
  if (hasCustomerSelection) {
    ...
    const [summaryResult, branchesResult, walletDeliveryResult, tenantResult] = await Promise.all([
      ...
      context.supabase.from("tenants").select("currency_code,location_validation_mode").eq("id", context.tenantId).maybeSingle(),
    ]);
    ...
    locationRequired = tenantResult.data?.location_validation_mode === "STRICT";
  ...
  return <main className="operations-page">
    ...
    >
      {customerSummaryFailed || !customerSummary ? <div className="operations-empty-state operations-search-empty"><h2>No se pudo abrir el cliente</h2><p>La tarjeta ya no está disponible o no pertenece a este negocio.</p></div> : <>
        ...
        </form></section> : <section className="operations-flow-step" aria-labelledby="purchase-confirm-title"><div className="operations-flow-intro"><p>Confirmación</p><h3 id="purchase-confirm-title">Revisa la compra</h3><span>El servidor volverá a calcular todo al confirmar.</span></div>{selectedPurchaseBranch && selectedAmountMinor !== null && purchasePreview ? <><section className="operations-preview"><div><span>Se agregarán</span><strong>{purchasePreview.stamps_awarded}</strong></div><div><span>Nuevo saldo</span><strong>{purchasePreview.projected_balance}</strong></div><div><span>Monto</span><strong>{formatPurchaseAmount(selectedAmountMinor, currencyCode)}</strong></div><p>Cálculo realizado por morrow</p></section><form action={confirmCustomerPurchase} className="operations-form"><input name="customerCardId" type="hidden" value={selectedCustomerCardId} /><input name="loyaltyCardId" type="hidden" value={selectedLoyaltyCardId} /><input name="branchId" type="hidden" value={selectedPurchaseBranch.branch_id} /><input name="amount" type="hidden" value={params.amount} /><label className="field"><span>Número de ticket</span><input name="ticketNumber" required autoComplete="off" /><small>Sucursal: {selectedPurchaseBranch.branch_name}</small></label><OperationLocationSubmit locationRequired={locationRequired}>Confirmar compra</OperationLocationSubmit></form></> : <p className="operations-alert is-error" role="alert">La previsualización ya no es válida. Vuelve al paso anterior para recalcular.</p>}</section>}
      </>}
