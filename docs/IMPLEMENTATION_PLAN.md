...
- [x] [Terminada] Add Admin-only branch editing for general data, location, geofence, proximity and status with tenant-scoped writes and explicit deactivation confirmation.
...
- [x] [Terminada] Generate a per-customer branded graphical stamp strip on the server at 1x/2x/3x, replace it with each signed pass update, and retain exact textual progress as the default compatibility fallback unless the Admin explicitly selects an image-only front.
...
- [x] [Terminada] Add a per-card option to show or hide graphical stamp circles over Apple Wallet's main image; when hidden, omit cyclic progress copy so the primary area contains only the image, while preserving existing-card defaults.
- [x] [Terminada] Make the optional Wallet card title accept an empty value when the uploaded logo already contains the business name, while retaining the 60-character maximum.
- [x] [Terminada] Add per-card custom stamp artwork, optional empty slots, bounded row distribution and drag positioning on a zoned Apple main-image preview, then reproduce the normalized layout in signed 1x/2x/3x strips.
