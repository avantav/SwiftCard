...
6. Task in progress: Implement redemption mobile flow.
7. Relevant files: `src/app/app/purchase/page.tsx`, `src/app/app/purchase/actions.ts`, `supabase/migrations/0013_purchase_operations.sql`, `src/app/app/scan/actions.ts`.
8. Commands already passed: `npm run lint` with one existing Next image warning, `npm run typecheck`, `npm run test:run` with 83 tests, and `npm run build`; build includes `/app/purchase`.
9. Pending commands: redemption RPC, AVAILABLE reward authorization, and one-reward-per-confirmation tests.
...
11. Next exact action: Add `app.redeem_reward` and `/app/redeem`, deriving tenant/staff/branch from the authenticated session, requiring an AVAILABLE reward, and atomically marking one reward REDEEMED.
