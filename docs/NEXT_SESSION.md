...
15. Immediate release step: configure restricted `NEXT_PUBLIC_GOOGLE_MAPS_API_KEY` locally/Hostinger, apply validated migrations `0043` through `0050` with approval, select every active branch location, activate strict geofencing and test an inside/outside operation from the HTTPS employee phone before continuing the lifetime-points/Apple smoke path.
16. Local validation completed: typecheck, lint, 216 Vitest tests, webpack production build and the complete disposable PostgreSQL migration/RLS suite through `0050` pass. Google Maps requires external key configuration before its real map tiles and place search can be visually smoke-tested.
17. Google Wallet generation remains pending. The Admin Android toggle is explicitly labeled as a conceptual preview, not a claim that Google pass issuance is implemented.
