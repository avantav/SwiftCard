...
13. Immediate release step: apply validated migrations `0043` through `0049` to hosted Supabase with approval, then smoke-test draft/resume/publish/register/handoff/accept/add-card/lifetime purchase/milestone/decimal report/automatic refresh/scan/search/customer-modal/redeem.
14. Local validation completed: typecheck, lint, 211 Vitest tests, webpack production build and the complete disposable PostgreSQL migration/RLS suite through `0049` pass. The point Web Card and Apple preview were reviewed at 375, 768, 1280 and 1440 px without overflow; temporary review code was removed.
15. Google Wallet generation remains pending. The Admin Android toggle is explicitly labeled as a conceptual preview, not a claim that Google pass issuance is implemented.
