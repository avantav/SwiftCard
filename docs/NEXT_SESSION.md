...
7. Scanner state: `@zxing/browser@0.1.5` with Node-compatible `@zxing/library@0.21.3` is route-scoped to the client scanner. Camera permission is explicit, environment-facing is preferred, and permission/device failures direct the employee to the integrated name/phone search. Manual token and URL entry were removed together with the separate `/app/customers` route.
