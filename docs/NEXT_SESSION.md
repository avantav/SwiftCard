...
10. Immediate release step: apply validated migrations `0043` through `0047` to hosted Supabase with approval, then smoke-test draft/resume/publish/register/handoff/accept/add-card/scan/search/repeat-delivery/customer-modal/redeem/purchase and signed-pass refresh.
11. Local validation completed: typecheck, lint, 206 Vitest tests, webpack production build and the complete disposable PostgreSQL migration/RLS suite through `0047` pass. The employee header was reviewed at 375, 768, 1280 and 1440 px without persistent online UI or overflow; temporary routes were removed.
12. Google Wallet generation remains pending. The Admin Android toggle is a preview of the common design, not a claim that Google pass issuance is implemented.
