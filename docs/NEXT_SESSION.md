...
2. Last completed feature: direct tenant-scoped Supabase Storage uploads for Apple Wallet design images.
3. Migration state: the user reports `0035` applied; migrations `0036_apple_wallet_tenant_designs.sql` and `0037_apple_wallet_storage_assets.sql` are local and must not be pushed remotely without explicit release approval.
...
8. Commands passed: `npm run lint`, `npm run typecheck`, 151 Vitest tests, `npm run db:verify-rls`, and `npm run build`.
9. Additional validation: Chrome review passed at 375, 768, 1280, and 1440 px; authorized local credentials produced a signed `.pkpass` ZIP. Real Apple device acceptance remains pending.
10. Immediate release step: review and apply `0036` plus `0037`, copy secrets to the deployment manager, enable a tenant design, upload assets, then download from an active Web Card on iPhone/Mac.
...
14. Continue only with additive migrations after `0037`; never modify an applied migration.
