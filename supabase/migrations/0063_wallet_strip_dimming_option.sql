-- Let tenant Admins preserve the original brightness of the Apple Wallet
-- primary image. Existing cards retain the current contrast treatment.

alter table public.loyalty_cards
  add column strip_dimming_enabled boolean not null default true;

create function app.save_loyalty_card_design_v4(
  target_card_id uuid,
...
declare
  save_result text;
begin
  if target_strip_dimming_enabled is null then
    return 'INVALID';
  end if;

  save_result := app.save_loyalty_card_design_v3(
    target_card_id,
  ...
  set strip_dimming_enabled = target_strip_dimming_enabled
...
revoke all on function app.save_loyalty_card_design_v4(
  uuid, boolean, text, text, text, text, text, text, text, text,
  integer, integer, integer, integer, integer, integer, boolean
) from public, anon;
grant execute on function app.save_loyalty_card_design_v4(
  uuid, boolean, text, text, text, text, text, text, text, text,
  integer, integer, integer, integer, integer, integer, boolean
) to authenticated;
