...
begin
  ...
  result_value := app.save_loyalty_card_design_v5(
    ...
    card_record.strip_dimming_enabled, false
  );
  if result_value <> 'SAVED' then
    raise exception 'Tenant Admin could not hide strip stamps';
  end if;

  if not exists (
    select 1 from public.loyalty_cards
    where id = card_record.id
      and strip_stamps_enabled = false
  ) then
    raise exception 'Strip stamp preference was not persisted';
  end if;
  ...
  result_value := app.save_loyalty_card_design_v5(
    ...
    card_record.logo_margin_y_percent, card_record.strip_scale_percent,
    card_record.strip_margin_x_percent, card_record.strip_margin_y_percent,
    card_record.strip_dimming_enabled, null
  );
  if result_value <> 'INVALID' then
    raise exception 'Null strip stamp preference was accepted';
  end if;
end;
$$;
...
begin
  ...
  if result_value <> 'INVALID' then
    raise exception 'Branch staff changed tenant strip stamp visibility';
  end if;
end;
$$;
